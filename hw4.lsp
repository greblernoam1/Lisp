...
(defun at-least-one-color (n c k)
  (cond ((> (- k c) 0) (append (list(node2var n c k))(at-least-one-color n (+ c 1) k)))
        ((eq (- k c) 0) (list (node2var n c k)))
    )
 )
...
(defun at-most-one-color (n c k)
  (cond ((> k c) (append (at-most-one-color-helper n c k (+ c 1))(at-most-one-color n (+ c 1) k));(at-most-one-color-one-color n (+ c 1) k)))
        ;((eq (- k c) 0) (list (node2var n c k)))
    )
...
(defun at-most-one-color-helper (n c k x)
  ...
  (cond ((>= k x)
        ...
        (cons
          ...
          (at-most-one-color-helper n c k (+ x 1)))
        )
    )
 ...
 (defun generate-edge-clauses-helper (e c k)
    (cond 
      ((>= k c) 
          ...
          (cons
            (append
              (list (-(node2var (car e) c k)))
              (list (-(node2var (car (cdr e)) c k)))
            )
